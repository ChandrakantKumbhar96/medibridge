        ...
        <div key={label} className="flex items-center">
          <div className="flex items-center gap-2">
            <div className={`flex h-8 w-8 items-center justify-center rounded-full text-sm font-semibold ${
              i <= current ? 'bg-primary-600 text-white' : 'bg-sand-200 text-sand-500'}`}>
              {i < current ? <Check size={16} /> : i + 1}
            </div>
            <span className={`hidden text-sm font-medium sm:block ${i <= current ? 'text-sand-800' : 'text-sand-400'}`}>{label}</span>
          </div>
          {i < steps.length - 1 && <div className="mx-3 h-px w-10 bg-sand-300" />}
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-sand-50">
      ...
      <div className="mx-auto max-w-5xl px-6">
        ...
        {step === 0 && (
          <div>
            <h1 className="text-3xl font-extrabold text-sand-900">Select Specialization</h1>
            <p className="mt-1 text-sand-500">Choose the medical specialty you need</p>
            <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {specialties.map((s) => (
                <button key={s.name} onClick={() => chooseSpecialty(s.name)}
                  className="rounded-2xl border border-sand-200 bg-white p-6 text-left shadow-sm transition hover:border-primary-600 hover:shadow-md">
                  <div className="text-3xl">{s.emoji}</div>
                  <div className="mt-4 text-lg font-bold text-sand-900">{s.name}</div>
                  <div className="text-sm text-sand-500">{s.doctors} doctors available</div>
                </button>
              ))}
        ...
        {step === 1 && (
          <div>
            <h1 className="text-3xl font-extrabold text-sand-900">Choose a Doctor</h1>
            ...
            <div className="mt-6 space-y-4">
              ...
              {doctors.length === 0 && (
                <Card className="text-sm text-sand-500">
                  No doctors are currently available in {specialty}.
                </Card>
              )}
            </div>
            <button onClick={() => setStep(0)} className="mt-5 text-sm font-medium text-sand-500 hover:text-primary-600">← Back</button>
          </div>
        )}

        {step === 2 && (
          <div>
            <h1 className="text-3xl font-extrabold text-sand-900">Select Date &amp; Time</h1>
            <p className="mt-1 text-sand-500">Pick a slot with {doctor?.full_name}</p>
            <Card className="mt-6">
              <label className="block text-sm font-semibold text-sand-700">Appointment Date</label>
            ...
            <div className="mt-5 flex justify-between">
              <button onClick={() => setStep(1)} className="text-sm font-medium text-sand-500 hover:text-primary-600">← Back</button>
        ...
        {step === 3 && (
          <div>
            <h1 className="text-3xl font-extrabold text-sand-900">Confirm Appointment</h1>
            ...
            <Card className="mt-6 max-w-lg">
              ...
              <dl className="mt-4 space-y-3 text-sm">
                <div className="flex justify-between"><dt className="text-sand-500">Specialty</dt><dd className="font-medium text-sand-800">{specialty}</dd></div>
                <div className="flex justify-between"><dt className="text-sand-500">Date</dt><dd className="font-medium text-sand-800">{date}</dd></div>
                <div className="flex justify-between"><dt className="text-sand-500">Time</dt><dd className="font-medium text-sand-800">{slot?.label}</dd></div>
                {reason && (
                  <div className="flex justify-between gap-6">
                    <dt className="text-sand-500">Reason</dt>
                    <dd className="text-right font-medium text-sand-800">{reason}</dd>
                  </div>
                )}
                <div className="flex justify-between border-t border-sand-100 pt-3">
                  <dt className="text-sand-500">Consultation Fee</dt>
                  <dd className="font-bold text-sand-900">₹{doctor?.consultation_fee}</dd>
                </div>
              </dl>
            </Card>
            <div className="mt-5 flex max-w-lg justify-between">
              <button onClick={() => setStep(2)} className="text-sm font-medium text-sand-500 hover:text-primary-600">← Back</button>
